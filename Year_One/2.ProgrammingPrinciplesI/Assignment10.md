...
Writing a library of utilities that you can subsequently use (over and over again) is an important and useful programming skill.  In this assignment, you will be creating a class (ArrayUtils) that you be able to use on subsequent assignments.  Writing these correctly now will enable you to save programming time later.  There is a lot of code which is loosely duplicated, so the assignment is not as intimidating as you may assume at first.
...
You will write an ArrayUtils class and populate the documented methods.  This javadoc page has all the information that is required...so make sure to study it.  You can write a main method for your personal testing, but the class will be tested using a different, exterior main method which you cannot access.
push ints
push dbls
pop ints
pop dbls
